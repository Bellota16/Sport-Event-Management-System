...
{
    $sql = "SELECT team_id, team_name FROM teams WHERE team_category='cricket'";
...
{
    $sql = "SELECT id, stadium_name FROM team_stadiums WHERE stadium_category='cricket'";
    $result = $conn->query($sql);
    $options = "";
    while ($row = $result->fetch_assoc()) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["add"])) { // Check if "Add Event" button is clicked
        ...
        $event_category = "cricket"; // Always set event_category to "cricket"
